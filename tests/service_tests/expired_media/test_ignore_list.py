...
async def test_get_ignore_list(
    ...
    expired_media_ignored_items: list[ExpiredMediaIgnoredItem],
):
...
async def test_add_ignored_media(
    ...
    updated_items_by_rating_key = {item.rating_key: item for item in expired_media_ignore_list_manager.get_all()}
    assert len(updated_items_by_rating_key) == len(new_items) + len(expired_media_ignored_items)
    for item in new_items + expired_media_ignored_items:
...
async def test_add_duplicate_media(
    ...
    original_items = expired_media_ignore_list_manager.get_all()

    new_name = random_string()
    item_to_duplicate = random.choice(expired_media_ignored_items)
...
async def test_delete_ignored_media(
    ...
    rating_keys_to_delete = [item.rating_key for item in random.sample(expired_media_ignored_items, random_int(1, 5))]
    expired_media_ignore_list_manager.delete(rating_keys_to_delete)

    updated_items = expired_media_ignore_list_manager.get_all()
    assert updated_items == [
        item for item in expired_media_ignored_items if item.rating_key not in rating_keys_to_delete
    ]
...
async def test_get_ignore_list_expired_items(
    ...
    items_to_expire = random.sample(expired_media_ignored_items, random_int(1, 5))
    ...
    assert updated_items == [item for item in expired_media_ignored_items if item.rating_key not in expired_rating_keys]
