...
from utils import PROJECT_DIR, logger, render_python_template

template = """# This file is auto-generated by generate_pydantic_exports.py
...
MODELS_PATH = PROJECT_DIR / "mediamanager" / "models"
...
class PyFile:
    ...
    def extract_classes(file_path: pathlib.Path) -> list[str]:
        ...
        return [class_ for class_ in classes if not class_.startswith("_")]
    ...
    def __post_init__(self):
        for file in self.directory.glob("*.py"):
            ...
            if len(pfile.classes) > 0:
                ...
                logger.debug(f"Skipping {file.name} as it has no classes")
...
def main():
    modules = find_modules(MODELS_PATH)

    for module in modules:
        logger.debug(f"Module: {module.directory.name}")
        for file in module.files:
            logger.debug(f"  File: {file.import_path}")
            logger.debug(f"    Classes: [{', '.join(file.classes)}]")
