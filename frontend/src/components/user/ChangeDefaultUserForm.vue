<template>
    <v-form v-model="formIsValid" @submit.prevent="handleSubmit">
        ...
        <v-text-field
            ...
            clearable
            :type="visible ? 'text' : 'password'"
            :rules="[validations.required, validations.isConfirmedPassword]"
            density="compact"
...
const emit = defineEmits(["changedUser"])
...
const form = reactive<ChangeDefaultUserForm>({
    email: "",
    password: "",
    passwordConfirm: "",
})
...
const validations = {
    ...
    isConfirmedPassword: (val: string) => val === form.password || "Passwords do not match",
}
...
async function handleSubmit() {
    ...
    const { data } = await api.post<Token>("/api/users/replace-default", formData);
    if (!data) {
        loading.value = false;
        errorMessage.value = "Invalid Email and/or Password. Please try again";
        return;
    }
...
<script lang="ts">
export type ChangeDefaultUserForm = {
    email: string;
    password: string;
